...
export function WordToPdfClient() {
  ...
  const startConversion = async (selectedFile: File) => {
    setStep(2);
    setProcessingState('Converting to PDF...');
  ...
  return (
    <ToolLayout 
      title="Word to PDF" 
      description="Convert Word documents to PDF exactly as they appear."
    >
      <div className="max-w-4xl mx-auto space-y-6 px-4 md:px-0 w-full overflow-x-hidden pb-12">
        ...
        {step === 1 && (
          <Card className="p-6 sm:p-12 border-dashed border-2 flex flex-col items-center justify-center relative group overflow-hidden bg-zinc-50/50 dark:bg-zinc-900/50 hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors min-h-[300px]">
            <input type="file" className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" accept=".doc,.docx,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document" onChange={handleFileChange} />
            ...
            <h3 className="text-xl font-bold mb-2 text-center">Upload Word Document</h3>
            <p className="text-muted-foreground text-sm text-center max-w-sm">
                Drag and drop your DOC or DOCX file here or click to browse.
            </p>
            {error && (
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <Card className="p-8 sm:p-12 text-center space-y-8 bg-zinc-50/50 dark:bg-zinc-900/50">
                   ...
                   <div className="flex flex-col sm:flex-row justify-center items-center gap-4 pt-4">
                       ...
                       <a href={downloadUrl} download={`${file?.name.replace(/\.docx?$/i, '') || 'converted'}.pdf`} className="w-full sm:w-auto">
