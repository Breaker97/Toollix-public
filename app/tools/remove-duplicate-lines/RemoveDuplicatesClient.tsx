...
export default function RemoveDuplicatesClient() {
  ...
  const handleProcess = () => {
    ...
    if (options.sort) {
      uniqueLines.sort((a, b) => a.localeCompare(b, undefined, { sensitivity: options.caseSensitive ? 'variant' : 'base' }));
    }
  ...
  return (
    <ToolLayout 
      title="Duplicate Purge Protocol" 
      description="Cleanse lists and datasets by removing redundant lines. Optimized for high-volume data cleaning and list normalization."
      fullWidth
    >
            ...
            <div className="suite-card rounded-[2.5rem] p-8 space-y-6 overflow-hidden relative">
              <div className="flex items-center justify-between border-b border-zinc-100 dark:border-zinc-800 pb-6">
                <div className="flex items-center gap-3">
                   ...
                   <h3 className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-zinc-500 italic">Input Matrix</h3>
                </div>
                <Button 
                   variant="ghost" 
                   size="icon" 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                 <div className="space-y-4">
                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2">Raw Data</label>
                    <textarea
                      className="w-full h-[400px] bg-zinc-50/50 dark:bg-zinc-800/30 rounded-2xl p-6 border border-zinc-100 dark:border-zinc-800 resize-none focus:outline-none font-mono text-[12px] leading-relaxed text-zinc-800 dark:text-zinc-200 caret-[#c5a059]"
                 ...
                 <div className="space-y-4">
                    <div className="flex justify-between items-center px-2">
                       <label className="text-[10px] font-black uppercase tracking-widest text-[#c5a059]">Unique Sequence</label>
                    ...
                    <textarea
                      readOnly
                      className="w-full h-[400px] bg-[#c5a059]/5 dark:bg-[#c5a059]/10 rounded-2xl p-6 border border-[#c5a059]/20 resize-none focus:outline-none font-mono text-[12px] leading-relaxed text-[#c5a059]"
            ...
            <div className="flex flex-col sm:flex-row gap-4">
               <Button 
                ...
                >
                  ...
                  {copied ? "COPIED TO CLIPBOARD" : "USE UNIQUE LIST"}
                </Button>
                <div className="flex items-center gap-4 px-8 h-20 rounded-[2rem] bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800">
                   <div className="flex flex-col">
                      <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">Purge Rate</span>
                      <span className="text-xl font-black text-[#c5a059]">
                        {input.trim() ? Math.round(((input.split("\n").length - output.split("\n").length) / input.split("\n").length) * 100) : 0}%
                      </span>
                   </div>
                </div>
            </div>
          ...
          <div className="lg:col-span-4 space-y-6 lg:sticky lg:top-24">
            ...
            <div className="suite-card rounded-[2.5rem] p-8 space-y-8 animate-in slide-in-from-right-4 duration-500">
               ...
               <div className="space-y-3">
                  {[
                    { key: "caseSensitive", label: "Case Sensitive", desc: "A vs a are different", icon: CaseSensitive },
                    { key: "trim", label: "Trim Whitespace", desc: "Ignore leading/trailing spaces", icon: AlignLeft },
                    { key: "sort", label: "Alphabetical Sort", desc: "Arrange output A-Z", icon: SortAsc },
                    { key: "removeEmpty", label: "Strip Empty Lines", desc: "Ignore blank entries", icon: X },
                  ].map((item) => (
                    <button
                      key={item.key}
            ...
            <div className="suite-card p-8 rounded-[2.5rem] bg-[#c5a059]/5 border border-[#c5a059]/10 space-y-4">
               <div className="flex items-center gap-3">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-widest text-[#c5a059]">Dedupe Logic</span>
               </div>
               <p className="text-[10px] font-medium text-slate-500 leading-relaxed uppercase tracking-wider italic">
                  Utilizes <code>Set</code> data structures for O(n) performance on large text volumes.
               </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
               <div className="suite-card p-6 rounded-3xl flex flex-col items-center text-center gap-2">
                  ...
                  <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">List Safe</span>
               </div>
               <div className="suite-card p-6 rounded-3xl flex flex-col items-center text-center gap-2">
